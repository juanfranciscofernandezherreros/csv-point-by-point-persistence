...
public class PointByPointPersistenceService {
    ...
    private void persistRow(PointByPointValue value) {
        ...
        rowUpserts.upsert(mapper.toEntity(value));

        long rowsRead = Math.max(job.getRowsRead(), value.getRowNumber());
        long rowsPersisted = rows.countByIdSourceEventId(value.getSourceEventId());
    ...
    private void complete(PointByPointValue value) {
        ...
        if (stored != expected) {
            throw new IllegalStateException("Database has " + stored + " rows for import "
